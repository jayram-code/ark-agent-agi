...
class MCPFileSystemTool:
    ...
    def ensure_kb_directory(self):
        ...
        if not os.path.exists(self.kb_directory):
            ...
            log_event(
                "MCPFileSystemTool", {"event": "created_kb_directory", "path": self.kb_directory}
            )
    ...
    def list_documents(self, pattern: str = "*.txt") -> List[Dict[str, Any]]:
        ...
        for file_path in files:
            try:
                ...
                log_event(
                    "MCPFileSystemTool",
                    {"event": "file_stat_error", "file": file_path, "error": str(e)},
                )

        log_event(
            "MCPFileSystemTool",
            {"event": "listed_documents", "count": len(documents), "pattern": pattern},
        )
        return documents
    ...
    def read_document(self, filename: str, max_chars: int = 10000) -> Dict[str, Any]:
        ...
        try:
            ...
            log_event(
                "MCPFileSystemTool",
                {"event": "document_read", "filename": filename, "size": len(content)},
            )
            ...
            log_event(
                "MCPFileSystemTool",
                {"event": "document_read_error", "filename": filename, "error": error_msg},
            )
            return {"success": False, "error": error_msg, "filename": filename}
    ...
    def search_documents(self, query: str, max_results: int = 5) -> List[Dict[str, Any]]:
        ...
        for doc_info in documents:
            try:
                        ...
                        matches.append(
                            {
                                ...
                                "content_preview": (
                                    doc_result["content"][:500] + "..."
                                    if len(doc_result["content"]) > 500
                                    else doc_result["content"]
                                ),
                                "size": doc_info["size"],
                ...
                log_event(
                    "MCPFileSystemTool",
                    {"event": "search_error", "file": doc_info["filename"], "error": str(e)},
                )
        ...
        log_event(
            "MCPFileSystemTool",
            {"event": "search_completed", "query": query, "results": len(results)},
        )
        return results
        ...
        for filename, content in sample_docs:
            ...
            try:
                with open(file_path, "w", encoding="utf-8") as f:
                    f.write(content)
                log_event(
                    "MCPFileSystemTool", {"event": "created_sample_doc", "filename": filename}
                )
            except Exception as e:
                log_event(
                    "MCPFileSystemTool",
                    {"event": "sample_doc_error", "filename": filename, "error": str(e)},
                )
