...
class RefundAgent(BaseAgent):
    ...
    async def receive(self, message: AgentMessage):
        ...
        # 3. Process or Queue
        if risk_level == "low":
            return await self.process_automated_refund(
                customer_id, order_id, amount, payment_method, reason, message.session_id
            )
        else:
            return await self.create_refund_ticket(
                customer_id,
                order_id,
                amount,
                payment_method,
    ...
    async def process_automated_refund(
        self, customer_id, order_id, amount, payment_method, reason, session_id
    ):
        log_event(
            "RefundAgent",
            {"event": "processing_automated_refund", "customer_id": customer_id, "amount": amount},
        )
        ...
        if refund_result["success"]:
            ...
            # Send confirmation
            await self.send_refund_confirmation(
                customer_id, order_id, amount, "refund_processed", session_id
            )
    ...
    async def create_refund_ticket(
        self, customer_id, order_id, amount, payment_method, reason, risk_level, session_id
    ):
        log_event(
            "RefundAgent",
            {
                "event": "creating_refund_ticket",
                "customer_id": customer_id,
                "risk_level": risk_level,
            },
        )
        ...
        confirmation_message = AgentMessage(
            ...
            payload={
                ...
                "variables": {
                    ...
                    "status": status,
                    "processing_time": (
                        "instant" if status == "refund_processed" else "1-2 business days"
                    ),
                },
            },
        )
