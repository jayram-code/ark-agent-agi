...
def create_ticket(intent, text, sentiment, category=None, tags=None, key_phrases=None, customer_id=None, priority_score=None, status='open'):
    init_db()
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    ...
    cursor.execute(
        """INSERT INTO tickets (intent, category, text, sentiment, tags, key_phrases, customer_id, priority_score, status) 
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        (intent, category, text, sentiment, tags_json, key_phrases_json, customer_id, priority_score, status)
    )
    conn.commit()
...
def derive_category(intent):
    ...
    if isinstance(intent, str):
        ...
        if any(word in intent_lower for word in ['refund', 'money', 'billing', 'payment']):
            ...
            return 'logistics'
        elif any(word in intent_lower for word in ['technical', 'broken', 'error', 'not working', 'issue']):
            return 'technical'
...
def get_tickets_by_category(category, limit=50):
    ...
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM tickets WHERE category = ? ORDER BY created_at DESC LIMIT ?", (category, limit))
